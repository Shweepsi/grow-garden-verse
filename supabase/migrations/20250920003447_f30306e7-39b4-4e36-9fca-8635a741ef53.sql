-- Fix gem drop clustering by improving pseudo-random algorithm
...
BEGIN
  ...
  -- IMPROVED PSEUDO-RANDOM: Better distribution for rapid harvesting
  DECLARE
    v_current_time timestamp with time zone := now();
    v_user_hash integer := (EXTRACT(EPOCH FROM v_current_time)::bigint % 2147483647)::integer;
    v_microseconds integer := EXTRACT(MICROSECONDS FROM v_current_time)::integer;
    v_plot_hash integer := (p_plot_number * 7919) % 1000000;
    v_user_id_hash integer := (EXTRACT(EPOCH FROM v_planted_time)::bigint + 
                              (hashtext(p_user_id::text) % 1000000) +
                              v_microseconds +
                              v_plot_hash) % 2147483647;
    v_combined_seed bigint := (v_user_id_hash::bigint * 16807 + v_user_hash::bigint * 48271) % 2147483647;
    v_pseudo_random numeric := (v_combined_seed % 1000000) / 1000000.0;
  BEGIN
