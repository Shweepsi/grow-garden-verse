-- Update harvest_plant_transaction to use fixed 15% gem probability
-- Simplified gem system: one harvest = 15% chance for 1 gem
...
DECLARE
  ...
  -- FIXED GEM PROBABILITY: 15% chance per harvest
  v_gem_chance constant numeric := 0.15;
BEGIN
  ...
  -- SIMPLIFIED GEM CALCULATION: Fixed 15% chance, deterministic
  -- Create deterministic pseudo-random based on harvest context
  DECLARE
    v_seed integer := (
  ...
  v_result = json_build_object(
    ...
    'gem_reward', v_calculated_gem_reward, -- Simplified 15% chance calculation
    ...
    'simplified_gems', true, -- Gems now use fixed 15% probability
    'gem_chance_used', v_gem_chance
  );
