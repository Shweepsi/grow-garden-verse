-- SOLUTION A: Backend-only gem calculation to fix duplication bug
-- Update harvest_plant_transaction to calculate gems independently from frontend
...
CREATE OR REPLACE FUNCTION public.harvest_plant_transaction(
  ...
  p_gem_reward integer, -- Frontend will pass 0, backend will calculate
...
DECLARE
  ...
  v_gem_chance numeric := 0;
  v_gem_multiplier numeric := 1;
BEGIN
  ...
  -- UNIFIED READINESS CHECK: Use the same logic as frontend
  ...
  -- Use plot.growth_time_seconds as source of truth (same as frontend)
  ...
  v_adjusted_growth_time := GREATEST(v_adjusted_growth_time, 1); -- Minimum 1 second
  ...
  -- Check if plant is ready using unified logic
    ...
    BEGIN
      RETURN json_build_object(
        ...
        'debug_info', json_build_object(
          ...
          'required_ms', v_required_time_ms,
          'adjusted_growth_time', v_adjusted_growth_time,
          'growth_multiplier', v_growth_multiplier,
          'base_growth_time', v_plot_row.growth_time_seconds
        )
      );
    END;
  END IF;
  
  -- SOLUTION A: Backend-only gem calculation with proper multipliers
  IF p_multipliers IS NOT NULL THEN
    -- Extract gem chance and gem multiplier from multipliers
    v_gem_chance := COALESCE((p_multipliers->>'gemChance')::numeric, 0);
    v_gem_multiplier := COALESCE((p_multipliers->>'gems')::numeric, 1);
    
    -- Calculate gem reward using backend probabilistic logic
    IF v_gem_chance > 0 THEN
      -- Use deterministic seed based on transaction for consistency
      DECLARE
        v_seed integer := (EXTRACT(EPOCH FROM now())::integer + p_plot_number + LENGTH(v_plant_row.display_name)) % 1000;
        v_pseudo_random numeric := (v_seed * 9301 + 49297) % 233280 / 233280.0;
      BEGIN
        IF v_pseudo_random < v_gem_chance THEN
          v_calculated_gem_reward := CEIL(1 * v_gem_multiplier);
        END IF;
      END;
    END IF;
  END IF;
  
  -- Calculate final values using exact same logic as frontend
  ...
  v_result = json_build_object(
    ...
    'gem_reward', v_calculated_gem_reward, -- Return backend-calculated gems
    ...
    'backend_gem_calculation', true -- Flag indicating gems calculated by backend
  );
