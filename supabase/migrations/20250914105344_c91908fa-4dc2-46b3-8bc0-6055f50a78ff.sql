-- Simplify gem drop logic: award at most 1 gem per harvest based on a single probability input (gemChance)
-- Remove gem multiplier effects to avoid complex stacking and potential duplication perceptions.
...
BEGIN
  ...
  -- BACKEND-ONLY GEM CALCULATION (Simplified): single probability, at most 1 gem
  IF p_multipliers IS NOT NULL THEN
    -- Extract gem chance from multipliers (0..1). Ignore any gem multipliers.
    v_gem_chance := COALESCE((p_multipliers->>'gemChance')::numeric, 0);
    v_gem_chance := GREATEST(0, LEAST(v_gem_chance, 1));
    
    IF v_gem_chance > 0 THEN
      -- Deterministic pseudo-random based on current time and plot/plant context
      DECLARE
  ...
  v_result = json_build_object(
    ...
    'gem_reward', v_calculated_gem_reward, -- Simplified backend-calculated gems
    ...
    'backend_gem_calculation', true -- Gems strictly backend-calculated
  );
