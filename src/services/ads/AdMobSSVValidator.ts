...
export class AdMobSSVValidator {
  private static readonly KEYS_URL = 'https://www.gstatic.com/admob/reward/verifier-keys.json';
  ...
  static async fetchPublicKeys(): Promise<Map<number, CryptoKey>> {
    ...
    if (this.cachedKeys.size > 0 && (now - this.lastKeyFetch) < this.CACHE_DURATION) {
    ...
    try {
      ...
      console.log(`AdMob SSV: Successfully cached ${this.cachedKeys.size} public keys`);
   ...
   */
  private static async verifySignature(url: URL, params: SSVParameters): Promise<boolean> {
    try {
  ...
  private static validateBusinessParameters(params: SSVParameters): {
    ...
    if (!params.ad_network || !params.ad_unit || !params.reward_amount || 
        !params.reward_item || !params.timestamp || !params.transaction_id) {
    ...
    if (Math.abs(now - timestamp) > fiveMinutes) {
      return { isValid: false, error: 'Timestamp too old or too far in future' };
    }
    ...
    if (params.ad_unit !== expectedAdUnit) {
      console.warn(`AdMob SSV: Unexpected ad_unit: ${params.ad_unit}, expected: ${expectedAdUnit}`);
