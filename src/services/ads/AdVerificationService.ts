...
export class AdVerificationService {
   ...
   */
  static async verifySession(userId: string, sessionId: string): Promise<{
    isValid: boolean;
    session?: any;
    error?: string;
  }> {
   ...
   */
  static verifyRewardIntegrity(session: any, expectedRewardType: string, expectedAmount: number): boolean {
    if (session.reward_type !== expectedRewardType) {
      console.warn(`AdMob: Reward type mismatch. Expected: ${expectedRewardType}, Got: ${session.reward_type}`);
      return false;
    }

    if (session.reward_amount !== expectedAmount) {
      console.warn(`AdMob: Reward amount mismatch. Expected: ${expectedAmount}, Got: ${session.reward_amount}`);
      return false;
    }
   ...
   */
  static async markSuspiciousSession(sessionId: string, reason: string): Promise<void> {
    try {
      ...
      console.warn(`AdMob: Session ${sessionId} marked as suspicious: ${reason}`);
    } catch (error) {
