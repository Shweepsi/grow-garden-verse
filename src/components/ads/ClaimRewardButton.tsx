...
export function ClaimRewardButton({ variant = 'default', className = '' }: ClaimRewardButtonProps) {
  ...
  const getButtonContent = () => {
    ...
    if (!rewardState.available && rewardState.timeUntilNext > 0) {
      ...
      return (
        <>
          <AlertCircle className="w-4 h-4 mr-2" />
          {variant === 'compact' ? timeFormatted : `Prochaine dans ${timeFormatted}`}
        </>
      );
    }
  ...
  const getButtonClassName = () => {
    const baseClasses = "transition-all duration-300 font-semibold shadow-lg transform-gpu";
    
    if (variant === 'floating') {
      const sizeClasses = "fixed bottom-20 right-4 z-50 rounded-full p-4 shadow-2xl";
      if (isDisabled) {
