  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md max-h-[90vh] bg-white/95 backdrop-blur-xl border border-orange-200/50 shadow-2xl shadow-orange-500/20 animate-in fade-in-0 zoom-in-95 duration-300 overflow-hidden flex flex-col">
        ...
        <div className="flex gap-3 pt-4 shrink-0 border-t border-gray-100 mt-4">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="flex-1 border-gray-200 hover:bg-gray-50 text-gray-700 font-medium transition-colors duration-200" disabled={isLoading}>
            Annuler
          </Button>
          
          <AdWatchButton watchState={watchState} selectedReward={!!selectedReward} dailyLimitReached={dailyLimitReached} onWatchAd={handleWatchAd} />
        </div>
      </DialogContent>
    </Dialog>;
}