...
export const PlotCard = memo(({
  ...
  return <div className={`aspect-square cursor-pointer transition-transform duration-200 relative group touch-target ${isPlanting ? 'pointer-events-none opacity-50' : ''} ${plot.unlocked && state === 'ready' ? 'hover:scale-105' : ''}`} onClick={handleClick}>
      <div className={`premium-card rounded-xl p-2 h-full flex flex-col items-center justify-center relative overflow-hidden ${plot.unlocked ? state === 'ready' ? 'ring-2 ring-yellow-400/50 shadow-yellow-400/20 shadow-lg' : 'hover:shadow-lg' : 'opacity-60'}`}>
        ...
        {!plot.unlocked ? <div className="text-center">
            ...
            {!canAffordUnlock && <p className="mobile-text-xs text-red-500 mt-1 font-medium">
                Garde 100🪙
              </p>}
        ...
        {plot.unlocked && state !== 'empty' && <div className="absolute top-1.5 right-1.5">
