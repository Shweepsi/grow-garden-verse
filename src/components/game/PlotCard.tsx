...
export const PlotCard = memo(({
  ...
  return (
    <div 
      className={`aspect-square cursor-pointer transition-transform duration-200 relative group touch-target ${
        isPlanting ? 'pointer-events-none opacity-50' : ''
      } ${
        plot.unlocked && (state === 'ready' || isAutoHarvestPlot) ? 'hover:scale-105' : ''
      }`} 
      onClick={handleClick}
    >
      <div className={`premium-card rounded-xl p-2 h-full flex flex-col items-center justify-center relative overflow-hidden ${
        plot.unlocked 
          ? isAutoHarvestPlot
            ? 'ring-2 ring-blue-400/50 shadow-blue-400/20 shadow-lg bg-gradient-to-br from-blue-50 to-indigo-50'
            : state === 'ready' 
            ...
            {isAutoHarvestPlot ? (
              ...
              <>
                ...
                {plantType ? (
                  ...
                  <p className="mobile-text-xs text-blue-600">
                    Configurer
                  </p>
                )}
              </>
