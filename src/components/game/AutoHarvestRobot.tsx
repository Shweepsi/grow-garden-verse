...
export const AutoHarvestRobot = ({
  ...
  const getPlantReward = (plantType: PlantType): number => {
    return EconomyService.getHarvestReward(plantType.level_required || 1, plantType.base_growth_seconds, playerLevel, multipliers.harvest, multipliers.plantCostReduction);
  };
      ...
      <DialogContent className="max-w-4xl h-[90vh] bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200 flex flex-col">
        ...
        <ScrollArea className="flex-1">
          <div className="space-y-4 pr-4">
            {/* Plante actuelle */}
            {currentPlantType && <div className="bg-green-100 border border-green-300 rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              ...
              return <Card key={plantType.id} className={`cursor-pointer transition-all duration-200 ${isCurrent ? 'ring-2 ring-green-400 bg-green-50 border-green-300' : isSelected ? 'ring-2 ring-blue-400 bg-blue-50 border-blue-300' : canAfford ? 'hover:shadow-lg hover:scale-105 border-gray-200' : 'opacity-60 border-gray-200'}`} onClick={() => canAfford && setSelectedPlant(plantType.id)}>
        ...
        <div className="flex gap-2 flex-shrink-0 pt-4 border-t">
          ...
          <Button onClick={handleConfirmSelection} disabled={!selectedPlant} className="flex-1 bg-blue-500 hover:bg-blue-600 text-white">
